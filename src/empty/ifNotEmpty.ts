...
type FinalType<T1, T2> = T1 extends null | undefined
  ? Extract<T1, null | undefined> | T2
  : T1 extends string
 ...
 * @returns 최종 값
 * ******************************************************************************************************************/
export function ifNotEmpty<T1, T2>(
  v: T1,
  v2: T2
): IsUnionInclude<T1, ''> extends true ? FinalTypeWithEmptyString<T1, T2> : FinalType<T1, T2> {
  return notEmpty(v) ? (v2 as any) : (v as any);
}
