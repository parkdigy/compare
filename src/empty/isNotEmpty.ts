...
type FinalTypeWithEmptyString<Type> = Type extends string
  ? Exclude<Type, ''>
  : Type extends object
    ? T<Type>
    : Extract<Type, NonNullable<Type>>;
 ...
 * @returns 값이 비어있는지 여부
 * ******************************************************************************************************************/
function isNotEmpty<T>(v: T): v is IsUnionInclude<T, ''> extends true ? FinalTypeWithEmptyString<T> : FinalType<T> {
  return !isEmpty(v);
}
