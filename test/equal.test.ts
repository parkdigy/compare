...
describe('equal', () => {
  it('Number 인 경우 테스트', () => {
    ...
    expect(equal(1, 1.0)).toBe(true);
    expect(equal(1 / 3, 1.0 / 3)).toBe(true);
  ...
  it('Date 인 경우 테스트', () => {
    ...
    expect(equal(new Date(), new Date(2025, 1, 1))).toBe(false);
  });

  it('Array 인 경우 테스트', () => {
    expect(equal([1, 2], [1, 2])).toBe(true);
    expect(equal([1, 1], [1, 2])).toBe(false);
  });

  it('Object 인 경우 테스트', () => {
    expect(equal({ a: '1' }, { a: '1' })).toBe(true);
    expect(equal({ a: '1' }, { a: '2' })).toBe(false);
    expect(equal({ a: '1' }, { a: '1', fun() {} })).toBe(false);
  });
